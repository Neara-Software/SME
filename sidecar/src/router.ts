...
export class MessageRouter {
  ...
  private webappClients: Map<string, WebSocket> = new Map();
  private webappsBySocket: Map<WebSocket, string> = new Map();
  private webappConnectedAt: Map<string, number> = new Map();
  private defaultWebappTag: string | null = null;
  private appletsByType: Map<string, WebSocket> = new Map();
  ...
  private pendingRequests: Map<number, { source: WebSocket; originalId: number; targetTag?: string }> = new Map();
  private nextProxyId: number = 1;

  // Track client subscribed to console log streaming
  private logStreamingClient: WebSocket | null = null;
   ...
   */
  private generateUniqueTag(base: string): string {
   ...
   */
  private failRequestsForTag(tag: string): void {
  ...
  routeMessage(source: WebSocket, message: WebSocketMessage): void {
    ...
    const msgType = (message as any).request?.type || (message as any).type || 'unknown';
    ...
    if (isTabsRequestMessage(message)) {
   ...
   */
  private handleTabsRequest(source: WebSocket): void {
    const tabs: WebappInfo[] = [];
   ...
   */
  private routeFromWebapp(source: WebSocket, message: WebSocketMessage): void {
    let msgType: string | undefined;

    if (isOneWayMessage(message)) {
   ...
   */
  private routeFromApplet(source: WebSocket, message: WebSocketMessage): void {
    // Extract target from routing metadata if present
    const routing = (message as any).routing as RoutingMeta | undefined;
    ...
    if (isRequestMessage(message)) {
      ...
      if (message.request.type === 'console_log') {
        const payload = message.request.payload as { subtype?: string };
   ...
   */
  private handleResponseFromWebapp(message: WebSocketMessage): void {
  ...
  private sendToClient(ws: WebSocket, message: WebSocketMessage): void {
    if (ws.readyState === WebSocket.OPEN) {
      const msgType = (message as any).response?.type || (message as any).type || 'unknown';
