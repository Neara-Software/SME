...
class DimCli {
  ...
  private responseResolver: ((result: unknown) => void) | null = null;
  private messageHandlers: Map<string, MessageHandler> = new Map();

  constructor(sidecarUrl: string = DEFAULT_SIDECAR_URL, targetTab: string | null = null) {
  ...
  async sendRequest(type: string, payload: Record<string, unknown>): Promise<unknown> {
...
// Command handlers
async function handleTest(cli: DimCli, args: string[]): Promise<CommandResult> {
...
async function handleAnalyze(cli: DimCli, args: string[]): Promise<CommandResult> {
  const moduleIndex = args.indexOf('--module');
  if (moduleIndex === -1 || !args[moduleIndex + 1]) {
    return { success: false, error: 'Missing --module argument' };
  }
...
async function handleSync(cli: DimCli, args: string[]): Promise<CommandResult> {
  const moduleIndex = args.indexOf('--module');
  if (moduleIndex === -1 || !args[moduleIndex + 1]) {
    return { success: false, error: 'Missing --module argument' };
  }
  ...
  let reportNames: string[] | undefined;
  ...
  const allTypeNamesOnDisk: string[] = [];
  ...
  const allReportNamesOnDisk: string[] = [];
...
async function handleStatus(cli: DimCli): Promise<CommandResult> {
...
async function handleHealthcheck(cli: DimCli): Promise<CommandResult> {
...
async function handleWorkspace(cli: DimCli, args: string[]): Promise<CommandResult> {
...
async function handleDocs(cli: DimCli, args: string[]): Promise<CommandResult> {
 ...
 */
async function handleLogs(cli: DimCli, args: string[]): Promise<CommandResult> {
  ...
  let entryCount = 0;
  cli.onMessage('console_log_entries', (payload: unknown) => {
    const data = payload as { entries: LogEntry[]; count: number; historical?: boolean };
    if (data.historical) {
  ...
  try {
    ...
    const startResult = await cli.sendRequest('console_log', {
      subtype: 'start',
      filter,
    }) as { success: boolean; message?: string; historicalCount?: number };
...
async function handleEval(cli: DimCli, args: string[]): Promise<CommandResult> {
  // Parse --code (required)
  ...
  try {
    const result = await cli.sendRequest('eval', { code, namespace }) as Record<string, unknown>;
...
async function handleTabs(cli: DimCli): Promise<CommandResult> {
  console.log('Listing connected tabs...');

  return new Promise((resolve) => {
    // Set up handler for tabs_list response
    cli.onMessage('tabs_list', (payload: unknown) => {
      cli.offMessage('tabs_list');

      const data = payload as { tabs: TabInfo[]; count: number; defaultTag: string | null };
 ...
 */
function parseCanonicalFieldName(canonicalName: string): { qualifiedType: string; qualifiedField: string } {
...
async function handleFieldUsages(cli: DimCli, args: string[]): Promise<CommandResult> {
  ...
  try {
    ...
    if (result.success) {
      const usages = result.usages as Array<Record<string, unknown>>;
      console.log(`\nFound ${usages.length} usage(s):\n`);

      for (const usage of usages) {
        const type = usage.type as string;
        const description = usage.description as string;
